...
//
//        [MBProgressHUD showError:@"productID不存在"];
...
//    [MBProgressHUD hideHUDForView:kWindow animated:YES];
    ...
    if(transaction.error.code != SKErrorPaymentCancelled) {
        ...
        if (transaction.error.code == 0) {
            ...
            [self filedWithErrorCode:IAP_FILEDCOED_APPLECODE error:nil];
        } else {
...
//
//    self.userId = [UserViewModel currenUser].uid;
//
...
#warning 在这里将凭证发送给服务器
    IAPReceiptModel* model = [IAPReceiptModel new];
    model.receipt = self.receipt;
//    [[JUserService new] checkIAPReceiptWithParameterModel:model success:^(JBaseResponsModel *responseModel, BOOL isCache) {
//        
//        if([responseModel.code integerValue]){
//            
//            [MBProgressHUD showSuccess:@"购买成功!"];
//            [self removeReceipt];
//            [self.delegate IApRequestDone];
//            
//        } else {
//            [MBProgressHUD showError:@"异常验证,请稍后重试"];
//        }
//        
